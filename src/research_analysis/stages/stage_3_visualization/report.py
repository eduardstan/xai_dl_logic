...
def create_simple_summary_report(df_summary: pd.DataFrame, output_dir: Path) -> Path:
    """
    Create a simple summary report as a fallback option.
    ...
    Returns:
        Path to the generated report file
    """
    report_path = output_dir / "simple_summary_report.md"
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    try:
        total_topics = len(df_summary)
        total_papers = df_summary['cluster_size'].sum() if 'cluster_size' in df_summary.columns else 0
        # Handle different column names for papers selected
        if 'papers_selected' in df_summary.columns:
            total_selected = df_summary['papers_selected'].sum()
        elif 'papers_selected_from_cluster' in df_summary.columns:
            total_selected = df_summary['papers_selected_from_cluster'].sum()
        else:
            total_selected = 0
        
        selection_ratio = f"{total_selected/total_papers:.2%}" if total_papers > 0 else "N/A"
        avg_cluster_size = f"{df_summary['cluster_size'].mean():.1f}" if 'cluster_size' in df_summary.columns else "N/A"
        max_cluster_size = f"{df_summary['cluster_size'].max()}" if 'cluster_size' in df_summary.columns else "N/A"
        min_cluster_size = f"{df_summary['cluster_size'].min()}" if 'cluster_size' in df_summary.columns else "N/A"
        
        content = f"""# Simple Analysis Summary
...
- **Selected Papers:** {total_selected:,}
- **Selection Ratio:** {selection_ratio}

## Topic Statistics

- **Average Cluster Size:** {avg_cluster_size}
- **Largest Topic:** {max_cluster_size}
- **Smallest Topic:** {min_cluster_size}

---

*Simple report generated by Research Analysis Framework*"""
        
        with report_path.open("w", encoding="utf-8") as f:
            f.write(content)
        
        logger.info(f"Simple summary report saved to: {report_path}")
        return report_path
        
    except Exception as e:
        logger.error(f"Failed to create simple summary report: {e}")
        raise 